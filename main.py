...
class CalcApp:
    def __init__(self, root):
        ...
        self.functions = {
            "Navigation": {
                ...
                "time_to_dest": (["Distance (nm)", "Speed (kt)"], "h"),
                "distance": (["Speed (kt)", "Time (h)"], "nm"),
                "ETA": (["Distance (nm)", "Speed (kt)"], "h"),
            ...
            "Wind & Heading": {
                "WCA": (["TAS (kt)", "Angle (°)", "Windspeed (kt)"], "°"),
                "true_hdg": (["Track (°)", "WCA (°)"], "°"),
                "mag_hdg": (["True Heading (°)", "Mag. Variation (°)"], "°"),
            },
            "Fuel & Range": {
    ...
    def show_main_menu(self):
        self.clear_frame()
        title = tk.Label(self.main_frame, text="Select a Calculation", font=("Arial", 14))
        title.pack(pady=10)

        for category, funcs in self.functions.items():
            cat_label = tk.Label(self.main_frame, text=category, font=("Arial", 12, "bold"), fg="darkblue")
            cat_label.pack(pady=(10, 0))

            for func_name, (params, _) in funcs.items():
                label = func_name.replace("_", " ").capitalize()
                btn = tk.Button(self.main_frame, text=label, width=35,
